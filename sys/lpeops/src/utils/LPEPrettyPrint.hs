  ...
  where
    ...
    showVisitor subExps expr =
        ...
        let str = case expr of
                    ...
                    (view -> Vsum _)                  -> "(" ++ List.intercalate "+" (map (\subExp -> customData subExp ++ "*" ++ show (multiplicity subExp)) subExps) ++ ")"
                    (view -> Vproduct _)              -> "(" ++ List.intercalate "*" (map (\subExp -> customData subExp ++ "^" ++ show (multiplicity subExp)) subExps) ++ ")"
