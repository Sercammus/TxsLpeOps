...
-- Wrapper around valExpr2dataExpr' so that it is easier to debug if necessary:
valExpr2dataExpr :: ValExpr.ValExpr VarId.VarId -> T2MMonad MCRL2Defs.DExpr
valExpr2dataExpr = valExpr2dataExpr' valExpr2dataExpr
...
valExpr2dataExpr' _f (ValExpr.view -> ValExpr.Vstrinre _string _regex) =
    -- WARNING! Regular expressions are considered to be out of scope, so we just always say that the regex applies!
    return $ MCRL2Defs.DBool True
