...
\item For each summand $s$ with $R_s \subset P$, choose some $\rho = [x \rightarrow h_x \;|\; x \in P \cup C_s]$ so that ${c_s}[\rho]$ is satisfiable and $c_t[\rho]$ is unsatisfiable for all $t \notin S_s$, where
